<div id='dgItemForm' class="easyui-dialog" style="width:750px;height:380px;left:10px;top:10px;padding:5px 5px"
		...
		$.ajax({
			...
			success: function(result){
				...
				if (result.success)	{
					...
					var html="<table class='table'><thead><th>X</th><th>Faktur</th><th>Tanggal</th><th>Termin</th><th>Jth Tempo</th>" +
						"<th>Jumlah</th><th>Saldo<th></thead><tbody>";
					for(i=0;i<fakturs.length;i++){
						html=html+"<tr>";
						html=html+"<td><input type='checkbox' name='faktur[]' id='faktur"+i+"' value='"+fakturs[i].purchase_order_number+"'></td>";
						html=html+"<td>"+fakturs[i].purchase_order_number+"</td><td>"+fakturs[i].po_date+"</td>";
