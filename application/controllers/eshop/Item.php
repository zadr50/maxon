...
class Item extends CI_Controller {
	...
	function view($item_id,$page="comments") {	
		...
		if($r=$this->db->select('item_picture,description,special_features')->where('item_number',$item_id)
			->get('inventory')->row()){
			if($r->item_picture!=''){
				$data['picture']=load_picture($r->item_picture);
				$data['title']=$r->description;
				$data['description']=$r->special_features;
			}
		}
		$data['message']='';
	...
	function search($cat_kode='') {
		...
		$price_max=$this->db->limit(20)->get("inventory")->row()->price_max;
