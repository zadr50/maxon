        <table id='dgCust' name='dgCust' class="easyui-datagrid"  
            data-options="iconCls: 'icon-search', fitColumns: true,
                singleSelect: true,  
                url: '<?=$url.'/customer/load_by_cust_no/'.$customer_number?>',
                toolbar:'#dgCustTool',
            " width="100%">
            <thead>
                <tr>
                    <th data-options="field:'zone',width:180">Zone</th>
...
<div id='dlgCust' class="easyui-dialog"  closed="true" buttons="#btnDlgCust">
        ...
        <table class='table'>
            <tr>
            <td>Zone</td><td><?=my_input3(array("field"=>"zone",
                "value"=>"","button"=>true,"func"=>"dlgzone_show()"))?>
            </td>
...
<script type="text/javascript">
    ...
    var _url="<?=$url?>";   
    var _cust_no="<?=$cust_no?>";
    var _mode="<?=$mode?>";
    function load_cust(){        
        var find=$("#find").val();
        $('#dgCust').datagrid({url:_url+"/customer/load_tarif/"+_zone+'/'+find});
