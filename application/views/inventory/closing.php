...
foreach($periode->result() as $row){
    $antrian=0;
    $s="select count(1) as z_cnt from zzz_stock_arsip where year_arc='$row->tahun' and month_arc='$row->bulan'";
    ...
    if($antrian==0){
        ...
        $msg="Running.. antrian tersisa [$antrian] baris.";
    }
	echo "<tr><td>$row->tahun</td><td>$row->bulan</td>
		...
		<input onclick='delete_arsip($row->tahun,$row->bulan);return false'type='button' class='btn btn-warning' value='Delete'>";
    if(user_id()=="admin"){
        echo " <input onclick='proses_arsip($row->tahun,$row->bulan);return false'type='button' class='btn btn-default' value='Run'>";
        
    }     
    echo "</td>";
    
    echo "<td><i>$msg</i></td></tr>";
}
?>
</tbody>
</table>
...
function proses_arsip(tahun,bulan){
    window.open(CI_ROOT+"inventory/closing_run/"+tahun+"/"+bulan,"_self");    
}
</script>