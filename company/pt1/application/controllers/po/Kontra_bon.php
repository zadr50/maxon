...
class Kontra_bon extends CI_Controller {
	...
	function save(){
		...
		if ($ok){
			echo json_encode(array('success'=>true,'nomor'=>$id,"msg"=>mysql_error()));
		} else {
    ...
    function browse_data($offset=0,$limit=10,$nama=''){
    	if($this->input->get('nomor')!=''){
    		$sql=$this->sql." and nomor='".$this->input->get('sid_nomor')."'";
		} else {
	...
	function select_faktur($supplier_number){
		...
		$sql="select purchase_order_number,po_date,due_date,amount,terms,i.supplier_number,s.supplier_name
