   ...
   $min_date=$this->session->userdata("min_date","");
	...
	if($posted) {
		echo link_button('UnPosting','','cut','false',base_url().'index.php/sales_dbmemo/unposting/'.$kodecrdb);		
	} else {
		echo link_button('Posting','','ok','false',base_url().'index.php/sales_dbmemo/posting/'.$kodecrdb);		
	}
    echo "<div style='float:right'>";
		echo link_button('Help', 'load_help(\'sales_dbmemo\')','help');		
		?>
...
<script type="text/javascript">
    ...
    function save_db_memo(){
		...
		function delete_memo() {
			var url='<?=base_url()?>index.php/sales_dbmemo/delete/'+$("#kodecrdb").val();
