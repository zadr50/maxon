...
class Browser
{
	...
	function render($as_output=false)
	{
		...
		if($tablename != '') $sql="select $select from $tablename where 1=1 ";
		...
		$query=$this->CI->db->query($sql);
			...
			$tmpl = array ( 'table_open'  => '<table width="100%"  class="table" id="'.$id.'" name="'.$id.'" >' );
 			...
 			if( $query )
			{
				foreach($query->result() as $row)
				{
					...
					$action="";
					if($show_edit_button)$action.="<a href='#'   value='$key_value' class='editLink btn btn-primary glyphicon glyphicon-pencil' title='Edit Row'></a>"; 
					if($show_delete_button)$action.="&nbsp <a href='#'   value='$key_value' class='deleteLink btn btn-warning glyphicon glyphicon-remove' title='Delete Row'></a>"; 
