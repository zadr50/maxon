...
//		$cm['is_active']=$data['is_active'];
	...
	function convert_to_customer($cust_id){
		if($cm=$this->get_by_id($cust_id)->row()){
			$data['customer_number']=$cm->cust_id;	
			$data['company']=$cm->cust_name;
			$data['street']=$cm->street;
			$data['first_name']=$cm->first_name;
			$data['last_name']=$cm->last_name;
			var_dump($data);
