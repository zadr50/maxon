...
<tr>
		...
		<td><input id='disc_prc_2' style='width:50px'/></td>
		<td><input id='disc_prc_3' style='width:50px'/>
...
<table class='table2' id='tbl_items'>
	<thead><tr>
		...
		<th>Date To</th><th>Disc%1</th><th>Disc%2</th><th>Disc%3</th>
		<th>Action</th>
		</tr>
	</thead>
	<tbody>
...
<script language="javascript">
	...
	function load_items(){
		...
		clear_grid();
		$.ajax({
            type: "GET",url: CI_BASE+"index.php/so/promosi/load_items/"+page,
			data: xparam,
	...
	function add_row(data){
		for(i=0;i<data.length;i++){
			...
			+"<td>"+row.disc_prc_2+"</td>"
			+"<td>"+row.disc_prc_3+"</td>"
	...
	function save_item(){
		...
		$.ajax({
            type: "GET",
			data: xparam,
			url: CI_BASE+"index.php/so/promosi/save/",
            success: function(msg){
				console.log(msg);
				var result = eval('('+msg+')');
				if (result.success){
