...
class Kereta extends CI_Controller {
    private $limit=100;
    private $table_name='al_ticket_kereta';
    private $file_view='travel/ticket_kereta';
    private $controller='travel/kereta';
    private $primary_key='id';
    private $sql="";
	private $title="DAFTAR TICKET KERETA";
	private $help="";
    function __construct()    {
       ...
       if($multi_company){
		...
		$this->load->model('travel/kereta_model');
		$this->fields=$this->kereta_model->fields;
    }
	...
	function save(){
		...
		if($mode=="add"){ 
			$ok=$this->kereta_model->save($data);
		} else {
			$ok=$this->kereta_model->update($id,$data);				
		}
		if($ok){
    ...
    function view($id,$message=null)	{
		...
		$model=$this->kereta_model->get_by_id($id)->row();
   ...
   function browse($offset=0,$limit=50,$order_column="",$order_type='asc'){
		...
		$data['fields']=$this->kereta_model->fields;
		$data['field_key']=$this->primary_key;
