	...
	<div class="form-group">
		...
		if($name=="relation"){
			...
			if($control=='radio'){
				...
				echo "<input type='text' class='form-control input-sm' name='$name' $readonly
				id='$name' placeholder='$caption' value='$value'>";
			} 
		}
		?>
...
<input type='hidden' name='mode' id='mode'	value='<?=$mode?>'>
 ...
 <script language='javascript'>
    ...
    var field_key_value='<?=$field_key_value?>';
    
	function edit_aed() {
	    field_key_value=$("#"+field_key).val();
	...
	function print_aed(){
		...
		add_tab_parent("Print: "+field_key_value,url);
	}
