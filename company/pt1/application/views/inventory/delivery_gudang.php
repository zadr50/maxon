<div class="thumbnail box-gradient">
    ...
    if($mode!="view")echo link_button('Save', 'save()','save');        
...
<form id="frmItem" method='post' action='<?=base_url()?>index.php/delivery_gudang/process'>
   <table width="100%" class="table">
       <tr>
            <td>No.PMB Receipt</td>
            <td><?php 
                echo form_input('ref1',$ref1,"id='ref1' onblur='load_items_recv();return false;'");
                if($mode!="view") echo link_button('','dlgrecv_po_show()',"search","false");     
...
?>
 ...
 <script language='javascript'>
    ...
    function load_items_recv(nomor)
    {
        $('#ref1').val(nomor)
        if(nomor!=""){
            url=CI_ROOT+"delivery_gudang/items_with_input/"+nomor;
            $('#dg').datagrid({url:url});
            $('#dg').datagrid('reload');            
        }
    }
