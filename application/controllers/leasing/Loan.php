...
*/		
	...
	function notify_gudang($loan_id){
		$this->loan_master_model->notify_gudang($loan_id);
	}
	function print_new_doc(){
		...
		$data["date_from"]=$d1;
		$data["date_to"]=$d2;
		$data["list_so"]=$this->sales_order_model->list_so_new($d1,$d2);
		$data["list_po"]=$this->purchase_order_model->list_po_new($d1,$d2);
		$data["list_do"]=$this->invoice_model->list_do_new($d1,$d2);
		$data["list_trx"]=$this->inventory_moving_model->list_trx_new($d1,$d2);
		$this->template->display_form_input("leasing/list_doc_new",$data);
	}
}
?>
