...
<?php
    ...
    if(!isset($dlgHeight))$dlgHeight=400;
    if($dlgHeight<100)$dlgHeight=400;
...
<?php 
    ...
    echo $extra_fields;
?>
	<table id="dg<?=$dlgId?>" class="easyui-datagrid"  data-options="toolbar: '', 
        singleSelect: true, fitColumns: true, pagination:true, url: ''">
		<thead>
			<tr>
			<?php 
			if($show_checkbox){
...
<div id="<?=$dlgTool?>" class='box-gradient'>
    ...
    ?>
    Sort: <?=form_dropdown($dlgId."_sort",$fields,"","id='".$dlgId."_sort' onchange='dlg".$dlgId."_sort_by();return false;'")?>
...
<script type="text/javascript">
	...
	function dlg<?=$dlgId?>_show(subEvent) {
        ...
        $('#dlg<?=$dlgId?>').window({left:50,top:50});  
