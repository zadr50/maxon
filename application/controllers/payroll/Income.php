...
class Income extends CI_Controller {
	...
	private $sql="select * from hr_jenis_tunjangan";
	...
	function add()	{
		$data=$this->set_defaults();           
	 	$this->_set_rules();
		$data['mode']='add';
        $this->template->display_form_input('payroll/income',$data);
	}
	function save(){		 
		...
		if($mode=="add"){ 
			...
			$ok=$this->jenis_tunjangan_model->update($id,$data);				
		}
	...
	function view($id,$message=null){
		 $id=urldecode($id);
		 $model=$this->jenis_tunjangan_model->get_by_id($id)->row();
	...
	function browse($offset=0,$limit=10,$order_column='kode',$order_type='asc')	{
        $data['caption']='DAFTAR JENIS  TUNJANGAN';
		$data['controller']='payroll/income';		
		$data['fields_caption']=array('Kode','Nama Tunjangan');
		$data['fields']=array('kode','keterangan');
		$data['field_key']='kode';
		$this->load->library('search_criteria');
		
		$faa[]=criteria("Kode","sid_kode");
		$faa[]=criteria("Nama Tunjangan","sid_nama");
		$data['criteria']=$faa;
        $this->template->display_browse2($data);            
    }
    function browse_data($offset=0,$limit=10,$nama=''){
		$sql="select t.* from hr_jenis_tunjangan t
		where 1=1";
	...
	function delete($id){
		$id=urldecode($id);
	 	$this->jenis_tunjangan_model->delete($id);
	 	$this->browse();
	}
}
