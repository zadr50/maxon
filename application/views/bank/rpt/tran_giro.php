...
<table cellspacing="0" cellpadding="1" border="0" width='800px'> 
     <tr>
        <td colspan='2'><h2><?=$model->company_name?></h2></td><td colspan='2'>
            <h2>DAFTAR  TRANSAKSI GIRO </h2></td>      
     </tr>
     <tr>
     ...
     <tr>
        <td colspan="8">
                <table class='titem'>
                <thead>
                    <tr><td>Tanggal</td><td>Voucher</td><td>Giro#</td><td>Cair?</td><td>Tgl.Giro</td>
                        <td>From Bank</td><td>Masuk</td><td>Keluar</td>
                       <td>Person</td><td>Faktur</td><td>Memo</td>
                    </tr>
                </thead>
                <tbody>
                <?
                ...
                foreach($rst_bank->result() as $r_bank){
                    ...
                    $sql="select i.check_date,i.voucher,i.trans_type,i.check_number,
                    i.from_bank,i.cleared_date,i.cleared,i.void,i.trans_id,
                    i.deposit_amount,i.payment_amount,i.supplier_number,i.payee,i.memo
                     from check_writer i 
                    where i.trans_type in ('cheque in','cheque out') and account_number='".$r_bank->bank_account_number."' 
                    and i.check_date between '$date1' and '$date2'  ";
                    $rst_so=$CI->db->query($sql);
                    $tbl="";
                     foreach($rst_so->result() as $row){
                        $faktur="";
                        $sql="select invoice_number from check_writer_items where trans_id='$row->trans_id'";
                        if($qcwi=$this->db->query($sql)){
                            foreach($qcwi->result() as $rcwi){
                                if($rcwi->invoice_number!=""){
                                    $faktur.=$rcwi->invoice_number.',';
                                }
                            }
                        } 
                        $tbl.="<tr>";
                        ...
                        $tbl.="<td>".($row->check_number)."</td>";
                        $tbl.="<td>".($row->cleared)."</td>";
                        $tbl.="<td>".($row->cleared_date)."</td>";
                        $tbl.="<td>".($row->from_bank)."</td>";
