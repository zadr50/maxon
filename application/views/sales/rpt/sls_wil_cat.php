...
<table cellspacing="0" cellpadding="1" border="0" width='800px'> 
     ...
     <tr>
     	<td colspan="8">
	     		...
	     		<thead>
	     			<tr>
	     				<td>Wilayah</td><td>Category</td><td>Qty</td>
     			...
     			sum(il.amount) as z_omset,sum(il.quantity) as zqty
    			...
    			$sql.=" group by r.region_name,stk.category";
				...
				$sub_total=0;
                 foreach($rst_so->result() as $row){
	             	...
	             	if($region_old!="".$row->region_name){
                 		$tbl.="<tr><td colspan=4><b>SUB TOTAL [$region_old]</b></td><td align=right><b>".number_format($sub_total,2)."</b></td></tr>";
...
//	                if($category!="")$sql.=" and stk.category='$category'";
	                if($region!="")$sql.=" and c.region='$region'";
					$sql.=" and cat.category='$row->category'";
					$sql.=" and region_name='$row->region_name'";
					$sql.=" group by il.item_number,il.description";
					
					if($qdtl=$this->db->query($sql)){
						foreach($qdtl->result() as $rdtl){
							$tbl.="<tr><td></td><td>$rdtl->item_number - $rdtl->description</td>
							<td align='right'>".number_format($rdtl->zqty)."</td><td></td><td align='right'>".number_format($rdtl->z_omset,2)."</td></tr>";
						}
					}
