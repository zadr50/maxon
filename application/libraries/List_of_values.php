...
class List_of_values
{
	...
	function render($setting){
	    /* 
		 * Library: list_of_values
		 * dlgBindId: id dialog lov,search,select dan lainnya, dipanggilnya 
		 * 		misal: nama_dlgbindid_show()
		 * 		apabila dlgId tidak diisi akan di cari this->get_by_name
		 * dlgId : apabila tidak diisi akan dibikin sama dg dlgBindId, 
		 * 		ada hubungan dg dlgUrlQuery apabil dlgId berbeda dg dlgBindId 
		 * 		dlgUrlQuery harus diisi karena tidak akan dicari di this->get_by_namme		
		 * dlgUrlQuery: untuk lookup tersendiri dari controllernuya, misal: coa/browse_data
		 * dlgTitle: untuk caption atau judul dialog pencarian ini
		 * dlgTool: id dari div untuk toolbar tambahan
		 * dlgWidth,dlgHeight : 
		 * modules: ?
		 * show_check1: tampilkan checkbox pilihan
		 * show_date_range: tampilkan range date from and to
		 * dlgCols: lihat dlgColsData
		 * dlgColsData: kolom field caption dan width, contoh: array("fieldname"=>"kode","Caption"=>Kode,"width"=>"80px")
		 * filter: query param filter lain tambahan dari dlgUrlQuery misal barang/browse_data?filter=value
		 * sysvar_lookup: varname dari tabel system_variables misal lookup_doc_type
		 * 
         */
		 ...
		 //reset seting dg new value seting parameter
        $this->setting=null;
		...
		if(isset($this->setting['dlgUrlQuery'])){
	        $url_data=base_url("index.php/".$this->setting['dlgUrlQuery']."/");
			
		} else {
	        $url_data=base_url("index.php/lookup/query/".$this->setting['dlgId']."/");			
		}
        ...
        $this->setting['dlgId']=$this->setting['dlgBindId'];    
            //untuk render view id dikembalikan ke dlgBindId
            //agar bisa dibuat function dlgId_show()
        //if(is_ajax()){
        //    echo json_encode($this->setting);
        //}   else {
    ...
    function get_by_name($what,$search=""){        
        ...
        switch($what){
            ...
            case "purchase_invoice":
                $sql="select distinct i.purchase_order_number,i.po_date,i.supplier_number,s.supplier_name,
                ...
                where i.potype='I' ";
                if($search!="")$sql.=" and (i.purchase_order_number='$search' 
                    or s.supplier_name like '%$search%' or i.supplier_number like '%$search%' )";
                if($sort_by!=""){
                    $sql.=" order by $sort_by";
                } else {
                    $sql.=" order by i.po_date desc";
...
//
            ...
            case ("req_no"):
                $sql="select purchase_order_number as req_no,po_date,doc_status,update_by,update_date, 
            ...
            case "inventory_account":    
                ...
                if($search!=""){
                	$sql.=" where (account like '$search%' or account_description like '%$search%')";
				}
                if($sort_by!=""){
                    $sql.=" order by $sort_by";
                } else {
